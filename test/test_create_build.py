...
def test_teamcity_build_process():
    ...
    # Создание проекта
    project_api = ProjectAPI(auth_api.base_url, auth_api.session)
    project_response = project_api.create_project(project_data)
    assert project_response.status_code == 200

    # Создание VCS Root
    vcs_root_api = VCSRootAPI(auth_api.base_url, auth_api.session)
    ...
    build_api = BuildAPI(auth_api.base_url, auth_api.session)
