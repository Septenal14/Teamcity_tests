...
class MenuListCreateElement(BasePage):
...
class CreateFormContainerElement(BasePage):
    def __init__(self, page):
        super().__init__(page)
    ...
    def input_project_details(self, name, project_id, description):
        with allure.step("Ввод данных для создания проекта"):
            self.actions.wait_for_selector(self.project_name_selector, timeout=100000)
...
class ProjectCreationPage(BasePage):
    def __init__(self, page):
        ...
        self.menu_list_create = MenuListCreateElement(page)
        self.create_form_container = CreateFormContainerElement(page)
